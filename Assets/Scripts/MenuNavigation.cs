...
///    猶予中でも常に反映される。「下へ移動したい」という意図した操作までブロックしていた
///    問題を修正するため。さらに、カーソル移動が実際に成立した（＝そのパネルで意味のある操作だった）瞬間に
///    <see cref="InputLock.Unlock"/> を呼び、決定のロックも即座に解除する。キーで操作し
///    始めた時点で「連打の勢い」ではなく「意図した操作」と判断できるため。
///    ただし <see cref="SceneTransition"/> のフェード演出中（<see cref="InputLock.NavigationAllowed"/> が
///    false の間）はカーソル移動・マウスホバーも含めて完全にブロックする。フェード中はまだ
